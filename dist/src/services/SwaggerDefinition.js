...
const deployUrl = config_1.CONFIG.DEPLOY_URL;
/**
 * @public
...
class SwaggerDefinition {
     ...
     */
    static addComponentSchema(schemaName, schmema) {
     ...
     */
    static addPath(pathName, path) {
        const existingPath = this.definition.paths[pathName];
        this.definition.paths[pathName] = existingPath ? Object.assign(Object.assign({}, existingPath), path) : path;
        for (const key in path) {
            if (path[key].responses) {
                path[key].responses = Object.assign(Object.assign({}, path[key].responses), this.standardResponses);
            }
        }
    }
    static save(filesrc) {
        ...
        const rootDir = server_1.ROOT_DIR;
...
SwaggerDefinition.standardResponses = {
    "401": {
...
SwaggerDefinition.definition = {
    ...
    servers: [
        { url: deployUrl || `http://localhost:${config_1.CONFIG.PORT}` }
    ],
    security: [{
            bearerAuth: [],
        }],
