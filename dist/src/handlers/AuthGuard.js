...
const OIDCIssuerTrust_1 = require("../services/OIDCIssuerTrust");
    ...
    static requireUserAuthentication(config = { sameUserAsId: false }) {
        let arr = [(req, res, next) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const issuer = decoded === null || decoded === void 0 ? void 0 : decoded.iss;
                    if (!issuer)
                        return next({ message: 'Missing issuer claim', status: 401 });
                    // Flag um zwischen internen und externen Tokens zu unterscheiden
                    let isExternalToken = false;
                    let verifiedToken;
                    if (issuer !== config_1.CONFIG.DEPLOY_URL && !config_1.CONFIG.ALLOWED_ISSUERS.includes(issuer)) {
                        isExternalToken = true;
                        const providers = getOIDCProviders();
                        const provider = (0, OIDCIssuerTrust_1.findTrustedProviderByIssuer)(providers, issuer);
                        if (!provider)
                            return next({ message: `Invalid issuer: ${issuer}! `, status: 401 });
                        // JWKS based verification replaces userinfo endpoint call.
                        try {
                            verifiedToken = (yield jwtService_1.jwtServiceInstance.verifyToken(token));
                        }
                        catch (err) {
                            console.error(err);
                    ...
                    if (!isExternalToken) {
                        ...
                        const preferredId = (verifiedToken === null || verifiedToken === void 0 ? void 0 : verifiedToken[trainingKey]) || (verifiedToken === null || verifiedToken === void 0 ? void 0 : verifiedToken[subKey]) || verifiedToken.sub;
                        try {
                            ...
                            if (byIdentity)
                                user = byIdentity;
                            if (!user) {
                                const byEmail = (yield UserBDTO_1.userBDTOInstance.findByAttributes({ email: preferredId }))[0] || (yield UserBDTO_1.userBDTOInstance.findByAttributes({ email: verifiedToken === null || verifiedToken === void 0 ? void 0 : verifiedToken.email }))[0];
                                if (byEmail)
                                    user = byEmail;
                            }
                            if (!user)
                        ...
                        if (config_1.CONFIG.OIDC_SYNC_GROUPS_ON_AUTH) {
                            try {
                                const groupsRaw = verifiedToken === null || verifiedToken === void 0 ? void 0 : verifiedToken[config_1.CONFIG.OIDC_CLAIM_GROUPS];
                                if (groupsRaw && typeof groupsRaw === 'string') {
                                    yield (0, OIDCGroupSyncService_1.syncGroupsAndMembershipsFromClaims)(user._id, groupsRaw);
                                }
                            }
                            catch (e) {
