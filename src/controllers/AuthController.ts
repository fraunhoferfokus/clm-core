            ...
            if (iss !== CONFIG.DEPLOY_URL) {
                ...
                try {
                    if (CONFIG.OIDC_SYNC_GROUPS_ON_REFRESH) {
                        ...
                        const decoded: any = idp_id_token
                            ? await verifyExternalToken(idp_id_token).catch(() => undefined)
                            : (idp_access_token ? await verifyExternalToken(idp_access_token).catch(() => undefined) : undefined)
                        if (decoded) {
